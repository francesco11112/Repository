...
main()
{
    ...
    dbase.Load("G:\CH4-pedroniTESTDATA1.in7");			  // obtain database information
    dbase.Info();
	...
	for (k=0; k<20; ++k)
	{
		...
		n[k]=(1/t[k])*m[k];										  // take the square of L11i invert multipy by uit's product and sum these over N
		println("n=",n[k]);
	}
	for (d=0; d<20; ++d)
	{
		f[d]= (sumc((x[0:][60+d]).^2)/39) ;	   		 // load the Git-1 data=---residuals3
		println("f=",f[d]);						// obtain si sqared, sum and find product with L11i
